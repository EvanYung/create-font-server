...
function getRecent(sucCallback, errCallback) {
  try {
    connection.query(
      "SELECT `code` FROM `fontlist` ORDER BY `createDate` DESC LIMIT 1",
      function (error, results, fields) {
        if (error) {
          errCallback && errCallback(error.sqlMessage);
          return;
        }
        sucCallback(fixData(results));
      }
...
function setNewCode(code, name, succ) {
  try {
    ...
    var queryStr =
      "INSERT INTO `fontlist` (`id`, `code`, `group`, `createDate`, `delFlag`) VALUES ('" +
      getId +
      "', '&#xe" +
      code +
      ";', '" +
